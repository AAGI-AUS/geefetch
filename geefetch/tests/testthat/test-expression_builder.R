...
test_that(".ee_const creates a constantValue node", {
  node <- .ee_const(42)
  expect_identical(node, list(constantValue = 42))
})

test_that(".ee_const handles different types", {
  expect_identical(.ee_const("hello")$constantValue, "hello")
  expect_true(.ee_const(TRUE)$constantValue)
  expect_null(.ee_const(NULL)$constantValue)
  expect_identical(.ee_const(list(1, 2))$constantValue, list(1, 2))
})
...
test_that(".ee_load_collection creates ImageCollection.load", {
  ...
  inv <- node$functionInvocationValue
  expect_identical(inv$functionName, "ImageCollection.load")
  expect_identical(inv$arguments$id$constantValue, "MODIS/061/MOD13A2")
})
...
test_that(".ee_filter_date creates Collection.filter with date filter", {
  ...
  inv <- node$functionInvocationValue
  expect_identical(inv$functionName, "Collection.filter")
...
test_that(".ee_select creates Image.select with band list", {
  ...
  inv <- node$functionInvocationValue
  expect_identical(inv$functionName, "Image.select")
  expect_identical(inv$arguments$bandSelectors$constantValue, list("B4", "B5"))
})
...
test_that(".ee_multiply creates Image.multiply for non-trivial factor", {
  ...
  inv <- node$functionInvocationValue
  expect_identical(inv$functionName, "Image.multiply")
})
...
test_that(".ee_add creates Image.add for non-trivial offset", {
  ...
  inv <- node$functionInvocationValue
  expect_identical(inv$functionName, "Image.add")
})
...
test_that(".ee_scale_offset chains multiply then add", {
  ...
  expect_identical(inner$functionName, "Image.multiply")
})
...
test_that(".ee_scale_offset skips identity operations", {
  ...
  expect_identical(node$functionInvocationValue$functionName, "Image.add")
...
test_that(".ee_feature_collection emits the invocation form the service accepts", {
  ...
  inv <- node$functionInvocationValue
  expect_identical(inv$functionName, "Collection")
...
test_that(".ee_sample_regions creates correct expression", {
  ...
  inv <- node$functionInvocationValue
  expect_identical(inv$functionName, "Image.sampleRegions")
  expect_true("image" %in% names(inv$arguments))
  expect_identical(
    inv$arguments$collection$functionInvocationValue$functionName,
    "Collection"
  )
  expect_identical(inv$arguments$scale$constantValue, 1000L)
...
test_that(".build_grid produces correct structure", {
  ...
  expect_identical(at$translateX, 138)
  expect_identical(at$translateY, -34)
  expect_gt(at$scaleX, 0)
  expect_lt(at$scaleY, 0)
...
test_that(".build_grid clamps to max_dim", {
  ...
  expect_lte(grid$dimensions$width, 512L)
  expect_lte(grid$dimensions$height, 512L)
})
...
test_that(".ee_dataset_image filters by bounds and mosaics scene collections", {
  ...
  expect_identical(built$band, "NDVI")
...
test_that(".ee_dataset_image takes the first image for global products", {
  ...
  expect_identical(built$band, "temperature_2m")
})
...
test_that(".ee_dataset_image derives SLGA bands from depth and stat", {
  ...
  expect_identical(built$band, "pHc_030_060_95")
  flat <- unlist(built$node, use.names = FALSE)

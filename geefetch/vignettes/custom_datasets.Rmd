...
with `read_gee()` and `collect_gee_data()` using `gee_register_dataset()`.
...
1. The **GEE collection ID** (find it in the
...
across sessions, add the `gee_register_dataset()` call to your script
or `.Rprofile`.
